...
func (s *Summary) serviceToString(svc *Service) string {
...
func (s *Summary) OneOutput(model model.Service) string {
	for _, svc := range s.Services {
		if svc.Name == model.Name.String() {
			ret := "\n──┤ Services Built … ├────────────────────────────────────────\n"
			ret += s.serviceToString(svc)
			return ret
		}
	}
