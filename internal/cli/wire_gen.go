...
func wireCmdUp(ctx context.Context, analytics3 *analytics.TiltAnalytics, cmdTags analytics2.CmdTags, subcommand model.TiltSubcommand) (CmdUpDeps, error) {
	...
	tiltServerOptions, err := apiserver.NewTiltServerOptions(ctx, tiltBuild)
	if err != nil {
		return CmdUpDeps{}, err
	}
	clientset, err := apiserver.NewTiltClientset(tiltServerOptions)
	if err != nil {
		return CmdUpDeps{}, err
	}
	apiserverController := apiserver.NewController(clientset, tiltServerOptions)
