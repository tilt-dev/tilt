...
func wireCmdUp(ctx context.Context, analytics3 *analytics.TiltAnalytics, cmdTags analytics2.CmdTags, subcommand model.TiltSubcommand) (CmdUpDeps, error) {
	...
	podlogstreamController := podlogstream.NewController(ctx, deferredClient, storeStore, client, podSource, clock)
	containerRestartDetector := kubernetesdiscovery.NewContainerRestartDetector()
	reconciler := kubernetesdiscovery.NewReconciler(deferredClient, scheme, client, containerRestartDetector, storeStore)
	runtime := k8s.ProvideContainerRuntime(ctx, client)
	...
	v3 := engine.ProvideSubscribers(headsUpServerController, tiltServerControllerManager, controllerBuilder, headsUpDisplay, terminalStream, terminalPrompt, serviceWatcher, buildController, configsController, triggerQueueSubscriber, eventWatcher, disableSubscriber, dockerComposeLogManager, analyticsReporter, analyticsUpdater, eventWatchManager, cloudStatusManager, dockerPruner, telemetryController, serverController, podMonitor, sessionController, subscriber, uiresourceSubscriber)
...
func wireCmdUpdog(ctx context.Context, analytics3 *analytics.TiltAnalytics, cmdTags analytics2.CmdTags, subcommand model.TiltSubcommand, objects []client.Object) (CmdUpdogDeps, error) {
	...
	podlogstreamController := podlogstream.NewController(ctx, deferredClient, storeStore, k8sClient, podSource, clock)
	containerRestartDetector := kubernetesdiscovery.NewContainerRestartDetector()
	reconciler := kubernetesdiscovery.NewReconciler(deferredClient, scheme, k8sClient, containerRestartDetector, storeStore)
