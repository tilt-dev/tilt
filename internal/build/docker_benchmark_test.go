...
func BenchmarkBuildTenStepsInOne(b *testing.B) {
	run := func() {
		...
		ref, err := f.b.BuildImageFromScratch(f.ctx, f.ps, f.getNameFromTest(), simpleDockerfile, nil, model.EmptyMatcher, steps, model.Cmd{}, false)
		if err != nil {
			b.Fatal(err)
		}
...
func BenchmarkIterativeBuildTenTimes(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		for j := 0; j < 10; j++ {
			ref, err = f.b.BuildImageFromExisting(f.ctx, f.ps, ref, nil, model.EmptyMatcher, steps, false)
			if err != nil {
				b.Fatal(err)
			}
		}
