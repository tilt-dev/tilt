...
func TestConditionalRunInFakeDocker(t *testing.T) {
	...
	_, err := f.b.BuildImageFromScratch(f.ctx, f.ps, f.getNameFromTest(), simpleDockerfile, []model.Mount{m}, model.EmptyMatcher, []model.Step{step1, step2}, model.Cmd{}, map[string]string{})
	if err != nil {
		t.Fatal(err)
	}
...
func TestAllConditionalRunsInFakeDocker(t *testing.T) {
	...
	_, err := f.b.BuildImageFromScratch(f.ctx, f.ps, f.getNameFromTest(), simpleDockerfile, []model.Mount{m}, model.EmptyMatcher, []model.Step{step1}, model.Cmd{}, map[string]string{})
	if err != nil {
		t.Fatal(err)
	}
