// Forked from https://github.com/tonistiigi/fsutil/pull/203
// Adapted upstream fsutil to fix a bug where
// directories aren't uploaded when their contents
// are uploaded.
...
var NewFS = fsutil.NewFS
var FollowLinks = fsutil.FollowLinks

type DirEntryInfo = fsutil.DirEntryInfo

type FilterOpt = fsutil.FilterOpt

type MapFunc = fsutil.MapFunc

// The result of the walk function controls
// both how WalkDir continues and whether the path is kept.
type MapResult = fsutil.MapResult

const (
	// Keep the current path and continue walking the file tree.
	MapResultKeep = fsutil.MapResultKeep
	...
	MapResultExclude = fsutil.MapResultExclude
	...
	MapResultSkipDir = fsutil.MapResultSkipDir
)
