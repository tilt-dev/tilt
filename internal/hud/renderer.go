...
func renderResource(r view.Resource) rty.Component {
	...
	const dashSize = 35
	l.Add(rty.ColoredString(fmt.Sprintf(" %s ", strings.Repeat("┄", dashSize-len(r.Name))), cLightText, tcell.ColorDefault))
	...
	if len(r.DirectoriesWatched) > 0 {
		...
		l = rty.NewLine()
		l.Add(rty.ColoredString(fmt.Sprintf("  (Watching %s)", strings.Join(dirs, " ")), cLightText, tcell.ColorDefault))
		lines.Add(l)
	}
	...
	if !r.LastDeployTime.Equal(time.Time{}) {
		if len(r.LastDeployEdits) > 0 {
			l = rty.NewLine()
			l.Add(rty.ColoredString(" Last Deployed Edits: ", cLightText, tcell.ColorDefault))
	...
	if !r.CurrentBuildStartTime.Equal(time.Time{}) {
		statusString := rty.ColoredString("In Progress", cPending, tcell.ColorDefault)
	...
	if !r.PendingBuildSince.Equal(time.Time{}) {
		statusString := rty.ColoredString("Pending", cPending, tcell.ColorDefault)
	...
	if !r.LastBuildFinishTime.Equal(time.Time{}) {
		shortBuildStatus := rty.ColoredString("OK", cGood, tcell.ColorDefault)
		if r.LastBuildError != "" {
			shortBuildStatus = rty.ColoredString("ERR", cBad, tcell.ColorDefault)
		}
	...
	l = rty.NewLine()
	l.Add(rty.ColoredString("  BUILD: ", cLightText, tcell.ColorDefault))
	...
	if r.PodStatus != "" {
		...
		l := rty.NewLine()
		l.Add(rty.ColoredString("    K8S: ", cLightText, tcell.ColorDefault))
		l.Add(rty.TextString(fmt.Sprintf("Pod [%s] • %s ago — ", r.PodName, formatDuration(time.Since(r.PodCreationTime)))))
		l.Add(rty.ColoredString(r.PodStatus, podStatusColor, tcell.ColorDefault))
		lines.Add(l)
	}
