...
func (c *Controller) Reconcile(ctx context.Context, req reconcile.Request) (reconcile.Result, error) {
	...
	if err != nil {
		...
		if err != nil {
			...
			c.addOrUpdateContainerWatches(ctx, streamName, stream, podNN, pod)
		}
	}
...
func (c *Controller) addOrUpdateContainerWatches(ctx context.Context, streamName types.NamespacedName, stream *v1alpha1.PodLogStream, podNN types.NamespacedName, pod *v1.Pod) {
	...
	for i, co := range containers {
		...
		if isActive {
			...
			if c.clock.Since(existing.doneWatchTime) < debounce {
				// TODO(nick): This should return a ctrl.Result that reschedules the reconcile.
				continue
			}
		}
