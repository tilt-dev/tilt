...
func toDisableSources(tlr *tiltfile.TiltfileLoadResult) map[string]*v1alpha1.DisableSource {
	result := make(map[string]*v1alpha1.DisableSource)
...
func toDisableConfigMaps(disableSources map[string]*v1alpha1.DisableSource) apiset.TypedObjectSet {
...
func toKubernetesApplyObjects(tlr *tiltfile.TiltfileLoadResult, disableSources map[string]*v1alpha1.DisableSource) apiset.TypedObjectSet {
	result := apiset.TypedObjectSet{}
	for _, m := range tlr.Manifests {
		if !m.IsK8s() {
			continue
		...
		ka.Spec.DisableSource = disableSources[disableConfigMapName(m)]
		result[name] = ka
	}
	return result
}
...
func toImageMapObjects(tlr *tiltfile.TiltfileLoadResult, disableSources map[string]*v1alpha1.DisableSource) apiset.TypedObjectSet {
	result := apiset.TypedObjectSet{}
	...
	for _, m := range tlr.Manifests {
		for _, iTarget := range m.ImageTargets {
			...
			im.Spec.DisableSource = disableSources[disableConfigMapName(m)]
			result[name] = im
		}
	}
	return result
...
func toCmdObjects(tlr *tiltfile.TiltfileLoadResult, disableSources map[string]*v1alpha1.DisableSource) apiset.TypedObjectSet {
	result := apiset.TypedObjectSet{}
	...
	for _, m := range tlr.Manifests {
		...
		cmd.Spec.DisableSource = disableSources[disableConfigMapName(m)]
		result[name] = cmd
	}
	return result
}
...
func toUIResourceObjects(tf *v1alpha1.Tiltfile, tlr *tiltfile.TiltfileLoadResult, disableSources map[string]*v1alpha1.DisableSource) apiset.TypedObjectSet {
	result := apiset.TypedObjectSet{}
