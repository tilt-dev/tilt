...
ifneq ($(CIRCLECI),true)
		...
		mkdir -p test-results
		gotestsum --format standard-quiet --junitfile test-results/unit-tests.xml -- ./integration -count 1 -v -p $(GO_PARALLEL_JOBS) -tags 'integration' -timeout 700s -run TestLiveUpdateAfter
endif
