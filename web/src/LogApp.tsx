...
var LogHeader = withRouter((props: any) => {
  ...
  let name = props.name
  let resourceNames = (props.resourceNames || []).filter(
    (resName: string) => resName !== "k8s_yaml"
  )
  let allOption = { value: "all", label: "All" }
  let options = [allOption].concat(
    resourceNames.map((resName: string) => {
  ...
  return (
    <header className="LogApp-header">
      <div className="LogApp-title">
        <Link to="/">Dashboard</Link>&nbsp;&gt;&nbsp;Logs:{" "}
      </div>
...
function selectStyles(): any {
  ...
  let option = (
    styles: any,
    {
      ...
      isFocused,
      isSelected,
    }: { data: any; isFocused: boolean; isSelected: boolean }
  ) => {
    let obj = { ...styles }
    if (data.value !== "all") {
      obj.paddingLeft = "1em"
...
class LogAppContents extends Component {
  ...
  refreshAutoScroll() {
    ...
    this._scrollTimeout = setTimeout(() => {
      let lastElInView =
        this._lastEl &&
        this._lastEl.getBoundingClientRect().top < window.innerHeight
