...
def k8s_resource(name: str, yaml: Union[str, Blob] = "", image: str = "", port_forwards: Union[str, int, List[int]] = [], extra_pod_labels: List[Dict[str, str]] = []) -> None:
  ...
  Args:
    ...
    extra_pod_labels: In addition to relying on Tilt's heuristics to automatically find K8S resources associated with this resource, a user may specify extra labelsets to force entities to be associated with this resource. An entity will be associated with this resource if it has all of the labels in at least one of the entries in this list (but still also if it meets any of Tilt's usual mechanisms).
  """
  pass
